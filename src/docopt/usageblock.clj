    ...
    (into (if (or pattern-parsing? (= "" no-arg-shorts)) [] [(re-tok "-([" no-arg-shorts "]+)")])
          (map (comp #(re-tok "-(" % ") ?(\\S+)")
                     (cond
                       ...
                       true                 #(str "[" (string/replace no-arg-shorts % "") "]*" %)))
...
'shorts/longs-re' are used to appropriately tokenize all possibly ambiguous options."
  ...
  (tokenize string (concat [[#"\.{3}"                             ::repeat]
                            [#"\|"                                ::choice]
                            [#"\[(?i)options\]"                   ::options]
                            [#"(\(|\[)"                           ::group]
                            [#"(\)|\])"                           ::end-group]]
...
(defn- find-option
  ...
  (let [takes-arg (not (empty? arg))
    ...
    (assert (and option (= takes-arg (:takes-arg option)))
            (str "PARSE ERROR: Usage line " lnum ": option '" (option name-key)
                 "' already defined"))
...
(defmultimethods make-node
  ...
  ::end-group (let [[group-type & children :as group] (peek stack)]
                (assert (= data (if (= group-type ::optional) "]" ")"))
                        (str "PARSE ERROR: Bad '" data "'" (if (number? line-number)
                                                             (str " in usage line " line-number))))
  ...
  (let [selectfn   #(map last (filter (comp (partial = %) first) tokens))
    ...
    (doseq [o (keys options)]
      (let [alt-o (assoc o :takes-arg (not (:takes-arg o)))
            linefn #(string/join ", " (sort (into #{} (map second (options %)))))]
        (assert (empty? (options alt-o))
                (str "PARSE ERROR Conflicting definitions of '"
                     (str \- (if (:long o) (str \- (:long o)) (:short o))) "': "
...
(defn- make-tree
  [names arguments tokenized-options]
  {:pre [(is (apply = names) "SYNTAX ERROR: Inconsistent program name in usage patterns: ")]}
  (let [tokens                       (tokenize-patterns arguments tokenized-options)
          tree                         (syntax-tree tokens)
          [options commands arguments] (collect-atoms tokens)
          accfn                        #(vector %3 (if (< 1 (occurs %3 tree)) %1 %2))]
      {:name       (first names)
       :tree       tree
       :shorts-re  (compile-short-options-re (filter :short tokenized-options) false)
       :longs-re   (compile-long-options-re  (filter :long  tokenized-options) false)
...
(defn parse
  ...
  [usage-lines tokenized-options]
  ; split the program name from the usage line
  (let [names&arguments (map #(string/split % #"\s*(\s+)" 2) usage-lines)]
    (make-tree (map first names&arguments)
